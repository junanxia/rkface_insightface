...
int find_text_pos(char c) {
	const char* p = strchr(m_char_table, c);
	if (p != NULL) {
		return p - m_char_table;
	}
	return -1;
}

static void set_argb8888_text(RK_U32 *buf, int sX, int sY, char* text, RK_U32 color) {
	// int font_pos;
	// int font_len;
	// if (type == 1) {
	// 	font_pos = 0;
	...
	// for (int k = 0; k < font_len; k++) {
	// 	for (int m = 0; m < 512; m++) {
	// 		char v = m_char_bits_zh[font_pos + k][m];
	...
	// 			int x = tmp % 64;
	// 			int y = tmp / 64;
	// 			int pix_ = k * 64 + x + y * 400; 
				
	// 			if (1 << n & v != 0) {
	// 				*(buf + pix_) = TEST_ARGB32_WHITE;	
	// 			} else {
	// 				*(buf + pix_) = color; 	
	// 			}
	// 		}
	...
	for (int k = 0; k < strlen(text); k++) {
		char c = text[k];
		int p = find_text_pos(c);

		if (p < 0) continue;

		for (RK_U32 i = 0; i < 16; i++)  {
			for(RK_U32 j = 0; j < 16; j++) {
				char v = m_char_bits[p][i + j * 16];

				int pix_ = k * 16 + i + j * 240;
				if (v == 0xFF) {
					*(buf + pix_) = TEST_ARGB32_WHITE; // color;    
				} else {
					*(buf + pix_) = color;   
				}
			}
...
RK_S32 test_rgn_overlay_line_process(int sX ,int sY,int type, int group) {
	...
	RGN_HANDLE RgnHandle = group * 5 + type;
	BITMAP_S stBitmap;
...
RK_S32 test_rgn_overlay_text_process(int sX ,int sY, char* text, int group, int type) {
	printf("========%s========\n", __func__);
	RK_S32 s32Ret = RK_SUCCESS;
	RGN_HANDLE RgnHandle = group * 5 + 4;
	BITMAP_S stBitmap;
	RGN_ATTR_S stRgnAttr;
	RGN_CHN_ATTR_S stRgnChnAttr;
	...
	*********************************************/
	...
	int color = type == 1 ? TEST_ARGB32_GREEN : TEST_ARGB32_BLUE;
	set_argb8888_text((RK_U32 *)ColorData, 0, 0, text, color);
...
RK_S32 rgn_overlay_release(int group) {	
	...
	for(RGN_HANDLE i = group * 5; i < group * 5 + 5; i++)
	{
